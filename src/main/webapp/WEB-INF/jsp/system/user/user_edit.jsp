						...
						<div class="col-xs-12">
								<form action="user/${msg }.do" name="userForm" id="userForm" method="post">
									...
									<table id="table_report" class="table table-striped table-bordered table-hover">
										...
										<tr>
											<td style="width:79px;text-align: right;padding-top: 13px;">校区:</td>
											<td>
												<select name="SCHOOL_ID" ID="SCHOOL_ID" style="width:98%;">
													<option value="">--请选择--</option>
													<c:forEach var="item" items="${schoolList }">
														<option value="${item.SCHOOL_ID }"
														<c:if test="${pd.SCHOOL_ID==item.SCHOOL_ID}">selected</c:if>>
														${item.NAME }</option>
													</c:forEach>
												</select>
												...
												name="currency" id="currency" data-placeholder="请选择"
												style="vertical-align: top; width: 200px;" <c:if test="${isView == '1'}">disabled </c:if>>
													<option value="">请选择</option>
													<c:forEach items="${sysCurrencys}" var="varTab">
														<option value="${varTab.GCODE}"
															<c:if test="${orderHead.currency==varTab.GCODE}">selected</c:if>>${varTab.GCODE}
															-- ${varTab.NAME}</option>
													</c:forEach>
											</select> --%>
